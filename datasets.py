...
def load_heart():
    df = pd.read_csv(os.path.join(DATA_DIR, 'heart.csv'),
                     dtype={'Age': np.int, 'Sex': str, 'ChestPainType': str, 'RestingBP': np.int, 'Cholesterol': np.int,
                            'FastingBS': str, 'RestingECG': str, 'MaxHR': np.int,
                            'ExerciseAngina': str, 'Oldpeak': np.float, 'ST_Slope': str, 'HeartDisease': np.int})
